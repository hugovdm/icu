...
static const char * const gSubTypes[] = {
    ...
    "stone",
    "ton",
    "", // FIXME(review): this should be generated by Java, from resource files.
    "percent", // FIXME(review): should this be here? both percent and permille
    "permille", // are also under "concentr", so need not be under "none"?
    "gigawatt",
    "horsepower",
    "kilowatt",
    "megawatt",
...
// FIXME(review):
// Can someone explain this please? I don't have a good grip on C++ "universal
// references" yet. And measunit.h only has:
//
//     MeasureUnit(const MeasureUnit &other);
//
// Documented as "Copy constructor". vscode resolves this to chat, though this
// has a &&other... and sets other.fImpl to nullptr? Doesn't seem like an
// innocuous copy if it destroys the original?
//
// I would have expected something like this:
//
//     fImpl = new MeasureUnitImpl(other.fImpl);
//
...
MeasureUnit &MeasureUnit::operator=(MeasureUnit &&other) noexcept {
    ...
    delete fImpl;
    fImpl = other.fImpl;
    // FIXME(review): Here it is again, looks to the naive eye like "destruction
    // of other"? Stealing its fImpl instead of copying it?
    other.fImpl = nullptr;
    fTypeId = other.fTypeId;
    fSubTypeId = other.fSubTypeId;
    return *this;
