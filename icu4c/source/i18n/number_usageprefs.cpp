...
void UsagePrefsHandler::processQuantity(DecimalQuantity &quantity, MicroProps &micros,
    ...
    // TODO(icu-units/icu#21): we don't support mixed units yet, so we're
    // looking only at the first Measure instance:
    ...
    // TODO(icu-units/icu#13): should we detect whether rounder has already been
    // overridden by the developer and then ignore the unit preference skeleton?
    // Or should we officially reject overrides, given that they can't be unit
    // usage / locale preference sensitive? For now, we take unit preference
    // skeleton as override whenever there is one:
