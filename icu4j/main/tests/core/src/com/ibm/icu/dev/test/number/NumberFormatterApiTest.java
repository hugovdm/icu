...
//             u"1 foot, 12 inches");
    ...
    @Test
    public void unitComposedCompoundMeasure() {
        String[] builtInCompoundUnits = {
            // "meter-per-square-second", // formatting "square-second" not supported yet
            "milligram-per-deciliter",
            // "millimole-per-liter", // formatting "millimole" not supported yet
            // "liter-per-100-kilometer", // formatting "100-kilometer" not supported yet
            "liter-per-kilometer",
            "mile-per-gallon",
        ...
        for (String identifier : builtInCompoundUnits) {
            for (ULocale locale : ULocale.getAvailableLocales()) {
                for (UnitWidth width : widths) {
                    MeasureUnit compoundUnit = MeasureUnit.forIdentifier(identifier);

                    // Split numerator and denominator
                    MeasureUnit numerator = null;
                    MeasureUnit denominator = null;
                    for (MeasureUnit subUnit : compoundUnit.splitToSingleUnits()) {
                        if (subUnit.getDimensionality() > 0) {
                            if (numerator == null) {
                                numerator = subUnit;
                            } else {
                                numerator = numerator.product(subUnit);
                            }
                        } else {
                            subUnit = subUnit.withDimensionality(subUnit.getDimensionality() * -1);
                            if (denominator == null) {
                                denominator = subUnit;
                            } else {
                                denominator = denominator.product(subUnit);
                            }
                        }
                    }

                    LocalizedNumberFormatter nfBuiltin = NumberFormatter.with()
                                                             .unit(compoundUnit) //
                                                             .unitWidth(width)
                                                             .locale(locale);
                    LocalizedNumberFormatter nfComposed = NumberFormatter.with()
                                                              .unit(numerator) //
                                                              .perUnit(denominator)
                                                              .unitWidth(width)
                                                              .locale(locale);

                    if (compoundUnit.getIdentifier() != "pound-force-per-square-inch") {
                        assertEquals(locale.toLanguageTag() + " " + identifier,
                                     nfBuiltin.format(1.2).toString(),
                                     nfComposed.format(1.2).toString());
                    } else {
                        if (!nfBuiltin.format(1.2).toString().equals(
                                nfComposed.format(1.2).toString())) {
                            System.out.println(locale + ", " + nfBuiltin.toSkeleton() + ", \"" +
                                               nfBuiltin.format(1.2) + "\"");
                            System.out.println(locale + ", " + nfComposed.toSkeleton() + ", \"" +
                                               nfComposed.format(1.2) + "\"");
                        }
                    }
                }
            }
